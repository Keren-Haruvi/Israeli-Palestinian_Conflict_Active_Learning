...
class ActiveLearningPipeline:
    ...
    def __init__(self, model,
        ...
        self.method = method
        self.majority_methods = self._choose_method()
    ...
    def _choose_method(self):
        """
        choose the sampling methods based on the specified method type
        - if 'random' is selected, it returns only the random sampling method
        - if 'all' is selected, it returns all available sampling methods
        """
        if self.method == 'random':
            majority_methods = [self._random_sampling] 
        elif self.method == 'all':
            majority_methods = [self._uncertainty_sampling, self._diversity_sampling, self._sentiment_sampling, self._longest_sampling, self._shortest_sampling, 
                       self._clustering_uncertainty, self._clustering_disagreement, self._unbalanced_sampling, self._mistake_sampling, self._query_by_committee]
        else:
            raise ValueError("Invalid method")
        return majority_methods
    ...
    def _majority_vote(self):
        """
        collects indices from various sampling methods, counts the occurrences of each index, 
        and selects the most common indices based on the budget for the current iteration
        """
        n_select = min(self.budget_per_iter, len(self.available_pool_indices))
        selected_indeces_list = []
        
        for method in self.majority_methods:
            selected_indeces_list.extend(method())
        
        counter = Counter(selected_indeces_list)
        most_common = counter.most_common(n_select)
        majority_vote_indices = [item[0] for item in most_common]
        
        self.available_pool_indices = np.setdiff1d(self.available_pool_indices, majority_vote_indices)
        return majority_vote_indices
    ...
    def run_pipeline(self):
        ...
        for iteration in range(self.iterations-1):
            selected_indeces = self._majority_vote()
            self.train_indices = np.concatenate([self.train_indices, selected_indeces]) 
            self.train_model()
...
def run_pipeline(path, features, methods_list, models, seeds, init_train_method='random', iterations=11, budget_per_iter=15,
    ...
    for feature in features:
        ...
        for model in models:
            ...
            for method in methods_list:
                ...
                scores_dict[feature][model][method] = np.mean(F1_list, axis=0)
...
if __name__ == "__main__":
    ...
    methods_list = ['random', 'all']
    models = ['RandomForestClassifier', 'LogisticRegression', 'SVC']
    seeds = [1,2,3,4,5]

    # accuracy_scores_dict = run_pipeline(path, features, methods_list, models, seeds, init_train_method='kmeans', use_shorter_aug=True)
    ...
    if plot_comparison:
        ...
        best_dict = run_pipeline(path, features, ['all'], models, seeds=seeds, init_train_method='kmeans', use_shorter_aug=True) 
        
        augmentation_accuracy_dict = {}
        augmentation_accuracy_dict['baseline'] = random_dict[features[0]][models[0]]['random']
        augmentation_accuracy_dict['majority'] = best_dict[features[0]][models[0]]['all']
